...
fun AccelorometerComposable(
    ...
    var allCurrentReadings by remember {
        mutableStateOf<List<AccelerationReading>>(listOf())
    }

    var singleCurrentAccelorometer by remember {
        mutableStateOf<AccelerationReading>(
            AccelerationReading(
            ...
            if (event == Lifecycle.Event.ON_DESTROY) {
                ...
                sensorEventListener = object : SensorEventListener {
                    ...
                    override fun onSensorChanged(event: SensorEvent) {
                        var singleCurrentAccelorometer = AccelerationReading(
                            timestampMillis = System.currentTimeMillis(),
                            x = event.values[0],
                            y = event.values[1],
                            z = event.values[2],
                        ...
                        insertionScope.launch {
                            if (isRecording && !executed) {
                                println("isRecording")
                                allCurrentReadings = allCurrentReadings + singleCurrentAccelorometer
