...
fun LightComposable(
    ...
    var allCurrentReadings by remember {
        mutableStateOf<List<LightSensorReading>>(listOf())
    }

    var singleCurrentLight by remember {
        mutableStateOf<LightSensorReading>(
            LightSensorReading(
            ...
            if (event == Lifecycle.Event.ON_DESTROY) {
                ...
                sensorEventListener = object : SensorEventListener {
                    ...
                    override fun onSensorChanged(event: SensorEvent) {
                        var singleCurrentLight = LightSensorReading(
                            timestampMillis = System.currentTimeMillis(),
                            lux = event.values[0],
                        )
                        println(singleCurrentLight)

                        var executed = false

                        insertionScope.launch {
                            if (isRecording && !executed) {
                                allCurrentReadings = allCurrentReadings + singleCurrentLight
