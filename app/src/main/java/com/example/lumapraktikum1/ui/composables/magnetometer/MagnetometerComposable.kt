...
fun MagnetometerComposable(
    ...
    var allCurrentReadings by remember {
        mutableStateOf<List<MagnetometerReading>>(listOf())
    }

    var singleCurrentMagnetometer by remember {
        mutableStateOf<MagnetometerReading>(
            MagnetometerReading(
            ...
            if (event == Lifecycle.Event.ON_DESTROY) {
                ...
                sensorEventListener = object : SensorEventListener {
                    ...
                    override fun onSensorChanged(event: SensorEvent) {
                        var singleCurrentMagnetometer = MagnetometerReading(
                            timestampMillis = System.currentTimeMillis(),
                            x = event.values[0],
                            y = event.values[1],
                            z = event.values[2],
                        ...
                        insertionScope.launch {
                            if (isRecording && !executed) {
                                println("isRecording")
                                allCurrentReadings = allCurrentReadings + singleCurrentMagnetometer
    ...
    Column(
        ...
        Spacer(modifier = Modifier.height(10.dp))
        MagnetometerDataComposable(allCurrentReadings, 25, 2)
